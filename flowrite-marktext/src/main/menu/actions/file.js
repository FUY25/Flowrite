...
const handleResponseForSave = async (e, { id, filename, markdown, pathname, options, defaultPath, flowrite }) => {
  const win = BrowserWindow.fromWebContents(e.sender)
  ...
  filePath = resolveMarkdownFilePath(filePath)
  return writeMarkdownFile(filePath, markdown, options, { flowrite })
    .then(() => {
...
ipcMain.on('mt::response-file-save-as', async (e, { id, filename, markdown, pathname, options, defaultPath, flowrite }) => {
  const win = BrowserWindow.fromWebContents(e.sender)
  let recommendFilename = getRecommendTitleFromMarkdownString(markdown)
  if (!recommendFilename) {
    recommendFilename = filename || 'Untitled'
  ...
  if (filePath && !canceled) {
    filePath = resolveMarkdownFilePath(filePath)
    writeMarkdownFile(filePath, markdown, options, { flowrite })
      .then(() => {
        if (!alreadyExistOnDisk) {
          ipcMain.emit('window-add-file-path', win.id, filePath)
          ipcMain.emit('menu-add-recently-used', filePath)
...
ipcMain.on('mt::rename', async (e, { id, pathname, newPathname }) => {
  ...
  const canonicalNewPathname = resolveMarkdownFilePath(newPathname)
  ...
  const doRename = async () => {
    try {
      ...
      e.sender.send('mt::set-pathname', {
        id,
        pathname: canonicalNewPathname,
        filename: path.basename(canonicalNewPathname)
      })
    } catch (err) {
  ...
  if (!await exists(canonicalNewPathname)) {
    ...
    const { response } = await dialog.showMessageBox(win, {
      ...
      defaultId: 1,
      message: `The file "${path.basename(canonicalNewPathname)}" already exists. Do you want to replace it?`,
      cancelId: 1,
      noLink: true
    })
