      ...
      <div class="editor-main">
        ...
        >
          <margin-anchor-highlights
            ref="marginAnchorHighlights"
            :editor-root="$el"
            :paragraph-index="marginParagraphIndex"
          ></margin-anchor-highlights>
          <margin-comment-dots
            ref="marginCommentDots"
            :editor-root="$el"
            :paragraph-index="marginParagraphIndex"
          ></margin-comment-dots>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      marginParagraphIndex: {
        list: [],
        byId: new Map()
      },
      marginParagraphObserver: null,
      marginParagraphRefreshRafId: null,
      flowriteMarginRefreshRafId: null,
      annotationLayoutSyncRafId: null,
      pathname: '',
      isShowClose: false,
  ...
  watch: {
    ...
    markdown: function (value, oldValue) {
      if (value !== oldValue) {
        this.scheduleFlowriteMarginRefresh()
  ...
  created () {
    this.$nextTick(() => {
      ...
      this.editorScrollContainer = container
      ...
      this.editor.on('change', changes => {
        ...
        this.scheduleFlowriteMarginRefresh()
        this.scheduleAnnotationLayoutSync()
        if (!this.showSideBar && !this.sourceCode && this.editorHasFocus) {
          this.$store.dispatch('SET_DISTRACTION_FREE_WRITING', true)
        }
      })
      ...
      this.editor.on('flowrite-selection-comment', selectionPayload => {
        this.$store.dispatch('OPEN_FLOWRITE_MARGIN_COMPOSER', selectionPayload)
      })
  ...
  methods: {
    ...
    createMarginParagraphIndex (list = []) {
      return {
        list,
    ...
    buildMarginParagraphIndex () {
      const editorRoot = this.getMarginParagraphRoot()
      if (!editorRoot) {
        return this.createMarginParagraphIndex()
      }

      const list = Array.from(editorRoot.querySelectorAll('.ag-paragraph[id]')).map(element => ({
        id: element.id,
        text: element.textContent || '',
        element
      }))

      return this.createMarginParagraphIndex(list)
    },

    refreshMarginParagraphIndex () {
      this.marginParagraphIndex = this.buildMarginParagraphIndex()
    },

    scheduleMarginParagraphIndexRefresh () {
      if (this.marginParagraphRefreshRafId) {
        window.cancelAnimationFrame(this.marginParagraphRefreshRafId)
      }

      this.marginParagraphRefreshRafId = window.requestAnimationFrame(() => {
        this.marginParagraphRefreshRafId = null
        this.refreshMarginParagraphIndex()
      })
    },

    scheduleFlowriteMarginRefresh () {
      if (this.flowriteMarginRefreshRafId) {
        window.cancelAnimationFrame(this.flowriteMarginRefreshRafId)
      }

      this.flowriteMarginRefreshRafId = window.requestAnimationFrame(() => {
        this.flowriteMarginRefreshRafId = null
        this.refreshMarginParagraphIndex()
        this.$nextTick(() => {
          const overlayRefs = [
            this.$refs.marginAnchorHighlights,
            this.$refs.marginCommentDots,
            this.$refs.marginCommentLayer
          ].filter(Boolean)

          overlayRefs.forEach(component => {
            if (typeof component.scheduleRefresh === 'function') {
    ...
    attachMarginParagraphObserver () {
      this.detachMarginParagraphObserver()
    ...
    detachMarginParagraphObserver () {
      if (this.marginParagraphObserver) {
        this.marginParagraphObserver.disconnect()
        this.marginParagraphObserver = null
      }
    ...
    getAnnotationRailWidth () {
      return getMarginRailWidthForViewport(window.innerWidth)
    },
    ...
    syncAnnotationLayout () {
      ...
      this.$store.commit('SET_LAYOUT', {
        ...
        marginRailWidth: this.getAnnotationRailWidth(),
        viewportWidth: window.innerWidth
      })
    },

    scheduleAnnotationLayoutSync () {
      if (this.annotationLayoutSyncRafId) {
        window.cancelAnimationFrame(this.annotationLayoutSyncRafId)
      }

      this.annotationLayoutSyncRafId = window.requestAnimationFrame(() => {
        this.annotationLayoutSyncRafId = null
        this.syncAnnotationLayout()
      })
    },
  ...
  beforeDestroy () {
    if (this.editorScrollContainer) {
      this.editorScrollContainer.removeEventListener('focusin', this.handleEditorFocusIn, true)
      this.editorScrollContainer.removeEventListener('focusout', this.handleEditorFocusOut, true)
    }
    this.detachMarginParagraphObserver()
    if (this.annotationLayoutSyncRafId) {
      window.cancelAnimationFrame(this.annotationLayoutSyncRafId)
    ...
    this.editor.destroy()
    this.editor = null
    this.editorScrollContainer = null
  }
}
</script>
