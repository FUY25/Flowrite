...
const hunspellLicensePath = path.join(rootDir, 'resources/hunspell_dictionaries/LICENSE-hunspell.txt')

const additionalPackages = {
  hunspell: {
    packageName: 'Hunspell',
    licenses: 'LGPL 2.1',
    // Some local workspaces do not include the optional Hunspell dictionary assets.
    // Keep license generation working by falling back to a short notice instead of crashing.
    licenseText: fs.existsSync(hunspellLicensePath)
      ? fs.readFileSync(hunspellLicensePath)
      : 'Hunspell dictionary license text is not present in this workspace.'
  }
}
